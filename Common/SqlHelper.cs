...
namespace ZGZY.Common
{
    ...
    public abstract class SqlHelper
    {
        ...
        public static readonly string wmrdc_connStr = ConfigurationManager.ConnectionStrings["wmrdc_connstr"].ConnectionString;
        public static readonly string connStr153 = ConfigurationManager.ConnectionStrings["conn153"].ConnectionString;
        ...
        /// <summary>
        /// 执行oracle增删改（对现有的数据库连接）【不常用】
        /// </summary>
        public static int ExecuteOracleNonQuery(string connectstring, CommandType commandType, string commandText, params OracleParameter[] paras)
        {
            OracleConnection conn = new OracleConnection(connectstring);
            OracleCommand cmd = new OracleCommand();
            PrepareOracleCommand(cmd, conn, null, commandType, commandText, paras);
            int val = cmd.ExecuteNonQuery();
        ...
        /// 返回DataTable oracle
        /// </summary>
        public static DataTable GetOracleDataTable(string connectionString, CommandType commandType, string commandText, params OracleParameter[] paras)
        {
            OracleCommand cmd = new OracleCommand();
        ...
        public static DataTable GetDataTable(string connectionString, CommandType commandType, string commandText)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                PrepareCommand(cmd, conn, null, commandType, commandText);
                using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                {
                    DataTable dt = new DataTable();
                    da.Fill(dt);
        ...
        private static void PrepareCommand(SqlCommand cmd, SqlConnection conn, SqlTransaction trans, CommandType commandType, string commandText)
        {
            try
            {
                if (conn.State != ConnectionState.Open)
        ...
        /// 准备一个待执行的oraCommand
        /// </summary>
        private static void PrepareOracleCommand(OracleCommand cmd, OracleConnection conn, OracleTransaction trans, CommandType commandType, string commandText, params OracleParameter[] paras)
        {
            try
            {
                if (conn.State != ConnectionState.Open)
