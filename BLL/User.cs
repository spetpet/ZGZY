...
namespace ZGZY.BLL
{
    ...
    public class User
    {
        ...
        {
            return dal.GetUserByClassId(id);
         
        }

        public string GetUserByClassIdtojson(int id)
        {
            return ZGZY.Common.JsonHelper.ToJson(dal.GetUserByClassId(id));
        ...
        public string GetPager(string tableName, string columns, string order, int pageSize, int pageIndex, string where, out int totalCount)
        {
            ...
            dt.Columns.Add(new DataColumn("UserClassId"));
            dt.Columns.Add(new DataColumn("UserClass"));
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                DataTable dtclass = new ZGZY.BLL.Class().GetClassByUserId(Convert.ToInt32(dt.Rows[i]["Id"]));
                ...
                dt.Rows[i]["UserClassId"] = ZGZY.Common.JsonHelper.ColumnToJson(dtclass, 0);
                dt.Rows[i]["UserClass"] = ZGZY.Common.JsonHelper.ColumnToJson(dtclass, 1);
            }
